...
export async function GET(request: Request) {
  ...
  try {
    ...
    const communityData = {
      ...
      image: communityInfo?.image || 'https://img.clerk.com/eyJ0eXBlIjoiZGVmYXVsdCIsImlpZCI6Imluc18yVWRFcXhoWlowYzJOdFZGR1dHeTBtNXZzbTkiLCJyaWQiOiJvcmdfMllYbjNaQmhZc1lTYVZSb2plakJqZTF5Wk9FIiwiaW5pdGlhbHMiOiJUIn0',
      createdBy: communityInfo.createdBy,
    };
    if (user?.id !== communityData.createdBy.id) {
      return new NextResponse(
        JSON.stringify({ name: "Unauthorized" }),
        { status: 401 }
      );
    }
    return new NextResponse(
    ...
    return new NextResponse(
      JSON.stringify({ name: "An error occurred while fetching the community data", error: error }),
      { status: 500 }
    );
  }
}